...
window.initializeVoiceClient = () => {
  ...
  class EaglercraftVoicePeer {
    constructor(client, peerId, peerConnection, offer) {
      ...
      this.peerConnection.addEventListener("icecandidate", (evt) => {
        if (evt.candidate) {
          self.client.iceCandidateHandler(
            self.peerId,
            JSON.stringify({
        ...
        this.peerConnection.createOffer(
          (desc) => {
            ...
            self.peerConnection.setLocalDescription(
              selfDesc,
              () => {
                self.client.descriptionHandler(
                  self.peerId,
                  JSON.stringify(selfDesc)
                );
                if (self.client.peerStateInitial != PEERSTATE_SUCCESS)
                  self.client.peerStateInitial = PEERSTATE_SUCCESS;
              },
              (err) => {
                console.error(
                ...
                if (self.client.peerStateInitial == PEERSTATE_LOADING)
                  self.client.peerStateInitial = PEERSTATE_FAILED;
          ...
          (err) => {
            console.error(
              'Failed to set create offer for "' + self.peerId + '"! ' + err
            );
            if (self.client.peerStateInitial == PEERSTATE_LOADING)
              self.client.peerStateInitial = PEERSTATE_FAILED;
            self.client.signalDisconnect(self.peerId);
      ...
      this.peerConnection.addEventListener("connectionstatechange", (evt) => {
        if (self.peerConnection.connectionState === "disconnected") {
          ...
          if (self.client.peerState != PEERSTATE_SUCCESS)
            self.client.peerState = PEERSTATE_SUCCESS;
        } else if (self.peerConnection.connectionState === "failed") {
          if (self.client.peerState == PEERSTATE_LOADING)
            self.client.peerState = PEERSTATE_FAILED;
          self.client.signalDisconnect(self.peerId);
        }
      });
    }
    ...
    setRemoteDescription(descJSON) {
      ...
      try {
          ...
          () => {
            if (remoteDesc.type == "offer") {
              self.peerConnection.createAnswer(
                (desc) => {
                  ...
                  self.peerConnection.setLocalDescription(
                    ...
                    () => {
                      ...
                      if (self.client.peerStateDesc != PEERSTATE_SUCCESS)
                        self.client.peerStateDesc = PEERSTATE_SUCCESS;
                    },
                    (err) => {
                      console.error(
                        'Failed to set local description for "' +
                      ...
                      if (self.client.peerStateDesc == PEERSTATE_LOADING)
                        self.client.peerStateDesc = PEERSTATE_FAILED;
                      self.client.signalDisconnect(self.peerId);
                    }
                  );
                },
                (err) => {
                  console.error(
                    'Failed to create answer for "' + self.peerId + '"! ' + err
                  );
                  if (self.client.peerStateDesc == PEERSTATE_LOADING)
                    self.client.peerStateDesc = PEERSTATE_FAILED;
                  self.client.signalDisconnect(self.peerId);
        ...
        console.error(
          'Failed to parse remote description for "' + self.peerId + '"! ' + err
        );
        if (self.client.peerStateDesc == PEERSTATE_LOADING)
          self.client.peerStateDesc = PEERSTATE_FAILED;
        self.client.signalDisconnect(self.peerId);
    ...
    addICECandidate(candidate) {
      try {
        ...
        if (this.client.peerStateIce != PEERSTATE_SUCCESS)
          this.client.peerStateIce = PEERSTATE_SUCCESS;
      } catch (err) {
        console.error(
          'Failed to parse ice candidate for "' + this.peerId + '"! ' + err
        );
        if (this.client.peerStateIce == PEERSTATE_LOADING)
          this.client.peerStateIce = PEERSTATE_FAILED;
  ...
  class EaglercraftVoiceClient {
        ...
        navigator.mediaDevices
          ...
          .then((stream) => {
            ...
            self.localMediaStream =
              self.microphoneVolumeAudioContext.createMediaStreamDestination();
            self.localMediaStreamGain =
              self.microphoneVolumeAudioContext.createGain();
            var localStreamIn =
              self.microphoneVolumeAudioContext.createMediaStreamSource(stream);
    ...
    resetPeerStates() {
      this.peerState =
        this.peerStateConnect =
        this.peerStateInitial =
        this.peerStateDesc =
        this.peerStateIce =
          PEERSTATE_LOADING;
    }
    ...
    signalConnect(peerId, offer) {
      ...
      try {
        ...
        const peerInstance = new EaglercraftVoicePeer(
          this,
          peerId,
          peerConnection,
          offer
        ...
        if (this.peerStateConnect != PEERSTATE_SUCCESS)
          this.peerStateConnect = PEERSTATE_SUCCESS;
      } catch (e) {
        if (this.peerStateConnect == PEERSTATE_LOADING)
          this.peerStateConnect = PEERSTATE_FAILED;
      }
    }
...
window.initializeLANServer = () => {
    ...
    constructor(client, peerId, peerConnection) {
      ...
      this.peerConnection.addEventListener(
        ...
        async (evt) => {
          ...
          self.client.remoteClientDataChannelHandler(
            self.peerId,
            self.dataChannel
          );
          self.dataChannel.addEventListener(
            "message",
            (evt) => {
  ...
  class EaglercraftLANServer {
    ...
    sendPacketToRemoteClient(peerId, buffer) {
      ...
      if (typeof thePeer !== "undefined" && thePeer !== null) {
        if (
          thePeer.dataChannel != null &&
          thePeer.dataChannel.readyState == "open"
        ) {
    ...
    signalRemoteConnect(peerId) {
      try {
        ...
        const peerInstance = new EaglercraftLANPeer(
          this,
          peerId,
          peerConnection
        );
